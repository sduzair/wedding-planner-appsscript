...
const SHEET_ID =
  '1R6ksk0qj4X6olJ2-FTnNy8T3i32OIB7P_HF_VWdOVls';
const CALENDAR_ID =
...
function log() {
  SpreadsheetApp.openById(SHEET_ID)
    ...
    .forEach((sheetRow, i) => {
      console.log('🚀 ~ .forEach ~ sheetRow:', sheetRow); // Calendar event if (sheetRow.cardStart && sheetRow.cardDue) { console.log('This is a Calendar Event');
      if (i === 0) {
        const createdEvent =
          CalendarApp.getOwnedCalendarById(
            CALENDAR_ID
          ).createEvent(
            sheetRow.cardName,
            sheetRow.cardStart,
